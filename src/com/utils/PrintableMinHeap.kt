...
import com.weightGraph.MinHeap
import kotlin.math.pow

// 我们的PrintableMaxHeap只能处理整数信息，所以继承的是MaxHeap<Comparable<Integer>>
object PrintableMinHeap {

    // 以树状打印整个堆结构
    fun <Item : Comparable<*>> treePrint(heap: MinHeap<Item>) {
        if (heap.size() >= 100) {
        ...
        for (i in 1..heap.size()) {
            ...
            assert(heap.data[i] as Float >= 0 && (heap.data[i] as Float) < 100)
            print(heap.data[i].toString() + " ")
        }
        println()
        println()
        var n: Int = heap.size()
        ...
        for (level in 0 until maxLevel) {
            ...
            run {
                ...
                while (indexCurLevel < curLevelNumber) {
                    line1 = putNumberInLine(heap.data[index] as Float, line1, indexCurLevel, curTreeMaxLevelNumber * 3 - 1, isLeft)
                    isLeft = !isLeft
                    index++
                    indexCurLevel++
                }
    ...
    private fun putNumberInLine(num: Float, line: String, indexCurLevel: Int, curTreeWidth: Int, isLeft: Boolean): String {
        var line = line
