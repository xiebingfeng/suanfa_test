...
object PrintableMaxHeap {

    // 以树状打印整个堆结构
    fun treePrint(heap: MaxHeap) {
        if (heap.size() >= 100) {
        ...
        for (i in 1..heap.size()) {
            ...
            assert(heap.data[i] as Int >= 0 && (heap.data[i] as Int) < 100)
        ...
        for (level in 0 until maxLevel) {
            ...
            run {
                ...
                while (indexCurLevel < curLevelNumber) {
                    line1 = putNumberInLine(heap.data[index] as Int, line1, indexCurLevel, curTreeMaxLevelNumber * 3 - 1, isLeft)
                    isLeft = !isLeft
                    index++
                    indexCurLevel++
                }
