...
object PrintableIndexMaxHeap {
    ...
    fun treePrint(heap: IndexMaxHeap) {
        ...
        for (i in 1..heap.size()) {
            ...
            assert(heap.index[i] as Int >= 0 && (heap.index[i] as Int) < 100)
            print(heap.index[i].toString() + " ")
        }
        println()
        println()
        var n: Int = heap.size()
        ...
        for (level in 0 until maxLevel) {
            ...
            run {
                ...
                while (indexCurLevel < curLevelNumber) {
                    line1 = putNumberInLine(heap.index[index] as Int, line1, indexCurLevel, curTreeMaxLevelNumber * 3 - 1, isLeft)
                    isLeft = !isLeft
                    index++
                    indexCurLevel++
                }
